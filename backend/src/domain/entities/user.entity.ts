...
import { CartDataInterface } from "../../interfaces/cartDataInterface";
...
export class UserEntity {
  ...
  public static fromObject(object: { [key: string]: any }): UserEntity {
    ...
    if (!_id && !id)
      throw CustomError.badRequest_400("UserEntity - id is missing"); // si el _id y id no vienen dará error
    if (!firstName)
      throw CustomError.badRequest_400("UserEntity - firstName is missing"); // si el firstName no viene dará error
    if (!lastName)
      throw CustomError.badRequest_400("UserEntity - lastName is missing"); // si el lastName no viene dará error
    if (!email)
      throw CustomError.badRequest_400("UserEntity - email is missing"); // si el email no viene dará error
    ...
    if (!password)
      throw CustomError.badRequest_400("UserEntity - password is missing"); // si el password no viene dará error
    if (!cartData)
      throw CustomError.badRequest_400("UserEntity - cartData is missing"); // si el cartData no viene dará error
