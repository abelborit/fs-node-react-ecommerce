...
export class AuthMiddleware {
  ...
  static async validateJWT(
    ...
    try {
      ...
      if (!payloadToken)
        throw CustomError.unauthorized_401("Invalid Token - AuthMiddleware"); // porque puede ser que no se firmó correctamente, no tiene la misma seed y que por ende no podemos confiar en ese token
      ...
      if (!payloadToken.email)
        throw CustomError.internalServer_500(
          "Email not in token - AuthMiddleware"
        );

      if (!payloadToken.id)
        throw CustomError.internalServer_500(
          "Id not in token - AuthMiddleware"
        );
      ...
      if (!user)
        throw CustomError.unauthorized_401(
          "Invalid token - user - AuthMiddleware"
        );
